...
MODULE inpout_2d
  ...
  USE parameters_2d, ONLY : channelSlope

  USE parameters_2d, ONLY : zeta, zeta1 , zeta2 , muBeta , ell 
  ...
  NAMELIST / rheology_parameters / rheology_model , mu , xi , channelSlope ,     &
       zeta , zeta1 , zeta2 , ell , muBeta ,     &
       tau , nu_ref ,    &
  ...
  SUBROUTINE read_param
    ...
    IF ( rheology_flag ) THEN
       ...
       ELSEIF ( rheology_model .EQ. 10 ) THEN
             ...
             WRITE(*,*) 'RHEOLOGY_MODEL =' , rheology_model , 'clear water roll wave model'

       ELSEIF ( rheology_model .EQ. 11 ) THEN

             WRITE(*,*) 'RHEOLOGY_MODEL =' , rheology_model , 'muI model by GrayEdwards'
...
210    CONTINUE
    ...
    ELSE
       ...
       IF ( (x0-(xllcorner + 0.5D0 * cellsize)) .GT. epsilon_topo ) THEN 
...
!        IF ( y0 .LT. yllcorner+0.5D0*cellsize ) THEN 
       IF ( (y0-(yllcorner+0.5*cellsize)) .GT. epsilon_topo ) THEN   
