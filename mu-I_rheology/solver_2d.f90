...
MODULE solver_2d
  ...
  USE parameters_2d, ONLY : zeta , zeta1 , zeta2 , ell, muBeta
  ...
  !> some first-order derivative matrix used for viscosity discretization
  REAL*8, ALLOCATABLE :: DHDX(:,:)
  REAL*8, ALLOCATABLE :: DHDY(:,:)
  REAL*8, ALLOCATABLE :: DMDX(:,:)
  REAL*8, ALLOCATABLE :: DMDY(:,:)
  REAL*8, ALLOCATABLE :: DNDX(:,:)
  REAL*8, ALLOCATABLE :: DNDY(:,:)
  REAL*8, ALLOCATABLE :: RHS_L1_QX(:,:)
  REAL*8, ALLOCATABLE :: RHS_L1_QY(:,:)
  REAL*8, ALLOCATABLE :: RHS_L1_RX(:,:)
  REAL*8, ALLOCATABLE :: RHS_L1_RY(:,:)
...
CONTAINS
  ...
  SUBROUTINE allocate_solver_variables
    ...
    ALLOCATE( DHDX( comp_cells_x , comp_cells_y ) )
    ALLOCATE( DHDY( comp_cells_x , comp_cells_y ) )
    ALLOCATE( DMDX( comp_cells_x , comp_cells_y ) )
    ALLOCATE( DMDY( comp_cells_x , comp_cells_y ) )
    ALLOCATE( DNDX( comp_cells_x , comp_cells_y ) )
    ALLOCATE( DNDY( comp_cells_x , comp_cells_y ) )
    ALLOCATE( RHS_L1_QX( comp_cells_x , comp_cells_y ) )
    ALLOCATE( RHS_L1_QY( comp_cells_x , comp_cells_y ) )
    ALLOCATE( RHS_L1_RX( comp_cells_x , comp_cells_y ) )
    ALLOCATE( RHS_L1_RY( comp_cells_x , comp_cells_y ) )
  ...
  SUBROUTINE deallocate_solver_variables
    ...
    DEALLOCATE( DNDX )
    DEALLOCATE( DNDY )
    DEALLOCATE( RHS_L1_QX )
    DEALLOCATE( RHS_L1_QY )
    DEALLOCATE( RHS_L1_RX )
    DEALLOCATE( RHS_L1_RY )
          ...
          vars_loop:DO i=1,n_vars
             ...
             IF ( comp_cells_x .GT. 1 ) THEN
                ...
                IF (j.EQ.1) THEN
                   ...
                   ELSEIF ( bcW(i)%flag .EQ. 10 ) THEN
                     !  x_stencil(1) = x_stag(1)
                      x_stencil(1) = -(1.0)*x_comp(1)
                      x_stencil(2:3) = x_comp(1:2)

                     !  qrec_stencil(1) = (bcW(i)%value)*(1.0+(bcW(i)%distAmp)*sin(2*3.1415926*t/(bcW(i)%distPeriod)))
                      qrec_stencil(1) = TDBC(bcW(i)%value, bcW(i)%distPeriod, bcW(i)%distAmp, t)
                      qrec_stencil(2:3) = qrec(i,1:2,k)
          ...
          IF ( comp_cells_x .GT. 1 ) THEN
             ...
             IF ( j.EQ.1 ) THEN
                ...
                DO i=1,n_vars
                   ...
                   ELSEIF ( bcW(i)%flag .EQ. 10 ) THEN
                     !  qrec_bdry(i) = (bcW(i)%value)*(1.0+(bcW(i)%distAmp)*sin(2*3.1415926*t/(bcW(i)%distPeriod)))
                      qrec_bdry(i) = TDBC(bcW(i)%value, bcW(i)%distPeriod, bcW(i)%distAmp, t)
  ...
  !******************************************************************************
  !> \brief Space discretization and time advancement for viscosity term
  !
  !> $\frac{\partial hu}{\partial t}=\frac{\partial \nu h^{3/2} \frac{\partial u}{\partial x} }{\partial x}$ extended to 2D
  !> Second-order central difference scheme for space. (maybe higher-order central scheme later)
  !> Second-order TVD RK scheme and third-order TVD RK scheme for time.
  !
  !> @author 
  !> Boyuan Yu
  !
  ...
  SUBROUTINE visc_diff_advan

  USE geometry_2d, ONLY : dx,dy ! dt is defined in this module
  ...
  IF (((bcW(1)%FLAG .NE. 10) .AND. (bcW(1)%FLAG .NE. 0)) .OR. (bcE(1)%FLAG .NE. 2)) THEN
  WRITE(*,*) 'ERROR: viscosity discretization only works for Dirichlet and TDBC inlet and outlet BC now'
  STOP
  ENDIF
  ...
  visc_coeff = (2.0)/(9.0)*((ell*SQRT(grav))/(muBeta))*   &
  (SIN(zeta)/SQRT(COS(zeta)))*((TAN(zeta2)-TAN(zeta))/(TAN(zeta)-TAN(zeta1)))

  ! first stage of RK2-TVD
  IF (comp_cells_y .GT. 1) THEN
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      IF (j .EQ. 1) THEN
        ! inlet BC X
        DHDX(j, k) = (q(1, 2, k) - TDBC(bcW(1)%value, bcW(1)%distPeriod, bcW(1)%distAmp, t))/(CDX)
        DMDX(j, k) = (q(2, 2, k) - bcW(2)%value)/(CDX)
        DNDX(j, k) = (q(3, 2, k) - 0.0D0)/(CDX)
        
      ELSEIF (k .EQ. 1) THEN
        ! outlet BC Y
        DHDY(j, k) = (2.0D0*q(1, j, k+1) - 2.0D0*q(1, j, k))/(CDY)
        DNDY(j, k) = (2.0D0*q(1, j, k+1) - 2.0D0*q(1, j, k))/(CDY)
        DMDY(j, k) = (2.0D0*q(1, j, k+1) - 2.0D0*q(1, j, k))/(CDY)

      ELSEIF (j .EQ. comp_cells_x) THEN
        ! outlet BC X
        DHDX(j, k) = (2.0D0*q(1, j, k) - 2.0D0*q(1, j-1, k))/(CDX)
        DMDX(j, k) = (2.0D0*q(2, j, k) - 2.0D0*q(2, j-1, k))/(CDX)
        DNDX(j, k) = (2.0D0*q(3, j, k) - 2.0D0*q(3, j-1, k))/(CDX)

      ELSEIF (k .EQ. comp_cells_y) THEN
        ! outlet BC Y
        DHDY(j, k) = (2.0D0*q(1, j, k) - 2.0D0*q(1, j, k-1))/(CDY)
        DNDY(j, k) = (2.0D0*q(1, j, k) - 2.0D0*q(1, j, k-1))/(CDY)
        DMDY(j, k) = (2.0D0*q(1, j, k) - 2.0D0*q(1, j, k-1))/(CDY)

      ELSE
        ! internal cells
        DHDX(j, k) = (q(1, j+1, k) - q(1, j-1, k))/(CDX)
        DMDX(j, k) = (q(2, j+1, k) - q(2, j-1, k))/(CDX)
        DNDX(j, k) = (q(3, j+1, k) - q(3, j-1, k))/(CDX)

        DHDY(j, k) = (q(1, j, k+1) - q(1, j, k-1))/(CDY)
        DMDY(j, k) = (q(2, j, k+1) - q(2, j, k-1))/(CDY)
        DNDY(j, k) = (q(3, j, k+1) - q(3, j, k-1))/(CDY)
      ENDIF
  ...
  ! assembly of the first-level RHS term
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      RHS_L1_QX(j,k) = visc_coeff*SQRT(q(1, j, k))*(DMDX(j,k)-(q(2, j, k)/q(1, j, k))*(DHDX(j,k)))
      RHS_L1_QY(j,k) = visc_coeff*SQRT(q(1, j, k))*0.5d0*(DMDY(j,k)-(q(2, j, k)/q(1, j, k))*   &
      (DHDY(j,k))+DNDX(j,k)-(q(3, j, k)/q(1, j, k))*(DHDX(j,k)))
      RHS_L1_RX(j,k) = visc_coeff*SQRT(q(1, j, k))*0.5d0*(DNDX(j,k)-(q(3, j, k)/q(1, j, k))*   &
      (DHDX(j,k))+DMDY(j,k)-(q(2, j, k)/q(1, j, k))*(DHDY(j,k)))
      RHS_L1_RY(j,k) = visc_coeff*SQRT(q(1, j, k))*(DNDY(j,k)-(q(3, j, k)/q(1, j, k))*(DHDY(j,k)))
    ENDDO
  ENDDO
  ...
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      ...
      IF (j .EQ. 1) THEN
        ! forward difference
        q0(2,j,k) = q(2,j,k) + dt*(RHS_L1_QX(j+1,k)-RHS_L1_QX(j,k))/(dx) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(dy)
        q0(3,j,k) = q(3,j,k) + dt*(RHS_L1_QY(j+1,k)-RHS_L1_QY(j,k))/(dx) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSEIF (j .EQ. comp_cells_x) THEN
        ! backward difference
        q0(2,j,k) = q(2,j,k) + dt*(RHS_L1_QX(j,k)-RHS_L1_QX(j-1,k))/(dx) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(dy)
        q0(3,j,k) = q(3,j,k) + dt*(RHS_L1_QY(j,k)-RHS_L1_QY(j-1,k))/(dx) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSE
        ! internal cells
        q0(2,j,k) = q(2,j,k) + dt*(RHS_L1_QX(j+1,k)-RHS_L1_QX(j-1,k))/(CDX) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(CDY)
        q0(3,j,k) = q(3,j,k) + dt*(RHS_L1_QY(j+1,k)-RHS_L1_QY(j-1,k))/(CDX) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ENDIF
      ...
      IF (k .EQ. 1) THEN
        ! forward difference
        q0(2,j,k) = q0(2,j,k) + dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k))/(dy)
        q0(3,j,k) = q0(3,j,k) + dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k))/(dy)
      ELSEIF (k .EQ. comp_cells_y) THEN
        ! backward difference
        q0(2,j,k) = q0(2,j,k) + dt*(RHS_L1_RX(j,k)-RHS_L1_RX(j,k-1))/(dy)
        q0(3,j,k) = q0(3,j,k) + dt*(RHS_L1_QY(j,k)-RHS_L1_QY(j,k-1))/(dy) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSE
        ! internal cells
        q0(2,j,k) = q0(2,j,k) + dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(CDY)
        q0(3,j,k) = q0(3,j,k) + dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ENDIF
      ENDIF
    ENDDO
  ENDDO

  ! second stage of RK2-TVD
  IF (comp_cells_y .GT. 1) THEN
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      IF (j .EQ. 1) THEN
        ! inlet BC X
        DHDX(j, k) = (q0(1, 2, k) -   &
        TDBC(bcW(1)%value, bcW(1)%distPeriod, bcW(1)%distAmp, t))/(CDX)
        DMDX(j, k) = (q0(2, 2, k) - bcW(2)%value)/(CDX)
        DNDX(j, k) = (q0(3, 2, k) - 0.0D0)/(CDX)
        
      ELSEIF (k .EQ. 1) THEN
        ! outlet BC Y
        DHDY(j, k) = (2.0D0*q0(1, j, k+1) - 2.0D0*q0(1, j, k))/(CDY)
        DNDY(j, k) = (2.0D0*q0(1, j, k+1) - 2.0D0*q0(1, j, k))/(CDY)
        DMDY(j, k) = (2.0D0*q0(1, j, k+1) - 2.0D0*q0(1, j, k))/(CDY)

      ELSEIF (j .EQ. comp_cells_x) THEN
        ! outlet BC X
        DHDX(j, k) = (2.0D0*q0(1, j, k) - 2.0D0*q0(1, j-1, k))/(CDX)
        DMDX(j, k) = (2.0D0*q0(2, j, k) - 2.0D0*q0(2, j-1, k))/(CDX)
        DNDX(j, k) = (2.0D0*q0(3, j, k) - 2.0D0*q0(3, j-1, k))/(CDX)

      ELSEIF (k .EQ. comp_cells_y) THEN
        ! outlet BC Y
        DHDY(j, k) = (2.0D0*q0(1, j, k) - 2.0D0*q0(1, j, k-1))/(CDY)
        DNDY(j, k) = (2.0D0*q0(1, j, k) - 2.0D0*q0(1, j, k-1))/(CDY)
        DMDY(j, k) = (2.0D0*q0(1, j, k) - 2.0D0*q0(1, j, k-1))/(CDY)

      ELSE
        ! internal cells
        DHDX(j, k) = (q0(1, j+1, k) - q0(1, j-1, k))/(CDX)
        DMDX(j, k) = (q0(2, j+1, k) - q0(2, j-1, k))/(CDX)
        DNDX(j, k) = (q0(3, j+1, k) - q0(3, j-1, k))/(CDX)

        DHDY(j, k) = (q0(1, j, k+1) - q0(1, j, k-1))/(CDY)
        DMDY(j, k) = (q0(2, j, k+1) - q0(2, j, k-1))/(CDY)
        DNDY(j, k) = (q0(3, j, k+1) - q0(3, j, k-1))/(CDY)
      ENDIF
  ...
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      RHS_L1_QX(j,k) = visc_coeff*SQRT(q(1, j, k))*(DMDX(j,k)-(q0(2, j, k)/q(1, j, k))*(DHDX(j,k)))

      RHS_L1_QY(j,k) = visc_coeff*SQRT(q(1, j, k))*   &
      0.5d0*(DMDY(j,k)-(q0(2, j, k)/q(1, j, k))*(DHDY(j,k))+DNDX(j,k)-(q0(3, j, k)/q(1, j, k))*(DHDX(j,k)))

      RHS_L1_RX(j,k) = visc_coeff*SQRT(q(1, j, k))*   &
      0.5d0*(DNDX(j,k)-(q0(3, j, k)/q(1, j, k))*(DHDX(j,k))+DMDY(j,k)-(q0(2, j, k)/q(1, j, k))*(DHDY(j,k)))

      RHS_L1_RY(j,k) = visc_coeff*SQRT(q(1, j, k))*(DNDY(j,k)-(q0(3, j, k)/q(1, j, k))*(DHDY(j,k)))
    ENDDO
  ENDDO  
  ...
  DO j = 1,comp_cells_x
    DO k = 1,comp_interfaces_y 
      ...
      IF (j .EQ. 1) THEN
        ! forward difference
        q(2,j,k) = 0.50d0*q(2,j,k) + 0.50d0*q0(2,j,k) + 0.50d0*dt*(RHS_L1_QX(j+1,k)-RHS_L1_QX(j,k))/(dx) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(dy)
        q(3,j,k) = 0.50d0*q(3,j,k) + 0.50d0*q0(3,j,k) + 0.50d0*dt*(RHS_L1_QY(j+1,k)-RHS_L1_QY(j,k))/(dx) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSEIF (j .EQ. comp_cells_x) THEN
        ! backward difference
        q(2,j,k) = 0.50d0*q(2,j,k) + 0.50d0*q0(2,j,k) + 0.50d0*dt*(RHS_L1_QX(j,k)-RHS_L1_QX(j-1,k))/(dx) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(dy)
        q(3,j,k) = 0.50d0*q(3,j,k) + 0.50d0*q0(3,j,k) + 0.50d0*dt*(RHS_L1_QY(j,k)-RHS_L1_QY(j-1,k))/(dx) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSE
        ! internal cells
        q(2,j,k) = 0.50d0*q(2,j,k) + 0.50d0*q0(2,j,k) + 0.50d0*dt*(RHS_L1_QX(j+1,k)-RHS_L1_QX(j-1,k))/(CDX) !+ dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(CDY)
        q(3,j,k) = 0.50d0*q(3,j,k) + 0.50d0*q0(3,j,k) + 0.50d0*dt*(RHS_L1_QY(j+1,k)-RHS_L1_QY(j-1,k))/(CDX) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ENDIF
      ...
      IF (k .EQ. 1) THEN
        ! forward difference
        q(2,j,k) = q(2,j,k) + 0.50d0*dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k))/(dy)
        q(3,j,k) = q(3,j,k) + 0.50d0*dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k))/(dy)
      ELSEIF (k .EQ. comp_cells_y) THEN
        ! backward difference
        q(2,j,k) = q(2,j,k) + 0.50d0*dt*(RHS_L1_RX(j,k)-RHS_L1_RX(j,k-1))/(dy)
        q(3,j,k) = q(3,j,k) + 0.50d0*dt*(RHS_L1_QY(j,k)-RHS_L1_QY(j,k-1))/(dy) !+ dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ELSE
        ! internal cells
        q(2,j,k) = q(2,j,k) + 0.50d0*dt*(RHS_L1_RX(j,k+1)-RHS_L1_RX(j,k-1))/(CDY)
        q(3,j,k) = q(3,j,k) + 0.50d0*dt*(RHS_L1_RY(j,k+1)-RHS_L1_RY(j,k-1))/(CDY)
      ENDIF
      ENDIF
    ENDDO
  ENDDO

  END SUBROUTINE visc_diff_advan


  !******************************************************************************
  !> \brief Function for TDBC
  !
  !> Sinusoidal incoming wave, fluctuating around the baseVal.
  !
  !> @author 
  !> Boyuan Yu
  !
  !******************************************************************************
  REAL*8 function TDBC(baseVal, wavePeriod, waveAmplitude, PHYTIME)
    IMPLICIT NONE
    REAL*8, INTENT(IN) :: baseVal, wavePeriod, waveAmplitude, PHYTIME
    TDBC = baseVal*(1.0D0+waveAmplitude*(SIN(2.0D0*3.1415926D0*PHYTIME/wavePeriod)))
  END FUNCTION TDBC
