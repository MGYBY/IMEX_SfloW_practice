    ...
    IF (rheology_flag) THEN
       ...
       ELSEIF ( rheology_model .EQ. 10 ) THEN
          ...
          IF ( REAL(mod_vel) .NE. 0.D0 ) THEN 
             ...
             ! IMPORTANT: grav3_surv is always negative 
             forces_term(2) = forces_term(2) -  ( u / mod_vel ) *               &
                  ( grav / xi ) * mod_vel ** 2 + grav * h * channelSlope
        ...
        ELSEIF ( rheology_model .EQ. 11 ) THEN
             forces_term(2) = forces_term(2) +  (h * grav * tan(zeta)) -              &
             (DCMPLX(tan(zeta1),0.D0)+             &
             (tan(zeta2)-tan(zeta1))/((muBeta*h)/(ell*(mod_vel/((grav*h)**(1.0/2.0))))+              &
             DCMPLX(1.0,0.D0)))*(u / mod_vel)*h*grav

             forces_term(3) = forces_term(3) -               &
             (DCMPLX(tan(zeta1),0.D0)+             &
             (tan(zeta2)-tan(zeta1))/((muBeta*h)/(ell*(mod_vel/((grav*h)**(1.0/2.0))))+              &
             DCMPLX(1.0,0.D0)))*(v / mod_vel)*h*grav                 
    ...
    IF (rheology_flag) THEN
       ...
       ELSEIF ( rheology_model .EQ. 10 ) THEN
          IF ( mod_vel .GT. 0.D0 ) THEN
             ...
             forces_term(2) = forces_term(2) -  ( u / mod_vel ) *               &
                  mu * h * ( - grav * grav3_surf ) !+ grav * h * channelSlope
